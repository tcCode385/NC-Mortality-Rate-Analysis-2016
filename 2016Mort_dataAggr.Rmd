...
```{r}
cdcMort_dat <- data.table::fread("./sourceData/CompressedMortality2016.txt", drop = 1)
cdcMort_dat
dim(cdcMort_dat)
```
...
```{r, message=FALSE}
# 2018 spreadsheets
rwj2018_rmd <- read_excel("./sourceData/2018 County Health Rankings Data - v2.xls", sheet = "Ranked Measure Data")       # only the Ranked Measure Data sheet
rwj2018_amd <- read_excel("./sourceData/2018 County Health Rankings Data - v2.xls", sheet = "Additional Measure Data")   # only the Additional Measure Data sheet
head(rwj2018_rmd, 10)
head(rwj2018_amd, 10)

# 2019 spreedsheet
rwj2019_rmd <- read_excel("./sourceData/2018 County Health Rankings Data - v2.xls", sheet = "Ranked Measure Data")       # only the Ranked Measure Data sheet
head(rwj2019_rmd, 10)
```
</br>
...
```{r}
# write.csv(mortality_nc16, "./output/nc2016.csv", row.names = FALSE)
...
```{r}
#  write.csv(mort_nc16_noNA, "./output/nc2016_noNA.csv", row.names = FALSE)
```
